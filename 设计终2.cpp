/*泻药，人在宿舍，刚下床铺，见此矩阵题目，回答一波*/ 
/*有点麻烦，没有使用函数，都写在main函数里面了*/ 
#include<stdio.h>
#include<stdlib.h>
int main()
{
	int ***p=NULL;//可进行多个矩阵运算 
	int **q=NULL;//存放运算后的矩阵
	...
	e=(int**)malloc(sizeof(int *) *(x+1));//多两个是为了存放运算时的中间值 
	for(i=0;i<=x;i++)
...
/*****************************又是快乐分割线********************************************/ 
	else
	{
		for(;1;1)
		{
			printf("进行两个矩阵的运算，输入1表示相加，输入2表示相减，输入3表示相乘,输入4表示中止程序,输入5结束并显示之前矩阵运算结果:");
			scanf("%d",&t); printf("\n");
			if(t==4)
				break;
			if(t==1)
...
//253行，可把我牛逼坏了，还没有写过这么长的程序，就是 现在还没有图形界面，等我先学习之后再来补充 
